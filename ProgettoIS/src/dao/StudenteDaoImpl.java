...
public class StudenteDaoImpl implements StudenteDaoInterface {
  ...
  public boolean registerUser(Studente user) {
    ...
    try {
      ...
      String query = "insert into studente(matricola,nome,cognome,password,codiceFiscale,email,dataNascita,luogoNascita,uniqueID)"
          + " values (?,?,?,?,?,?,?,?,?)"; 
  ...
  public boolean loginUser(Studente user) {
    ...
    try {
      ...
      // ritorna true se utente trovato
      status = rs.next(); 
  ...
  public ArrayList<Studente> getTirocinanti(String p) {
    ...
    try {
      ...
      con = DBConnection.createConnection();
      String query = "SELECT studente.matricola, studente.nome , studente.cognome , studente.codiceFiscale ,"
          + "studente.email ,studente.dataNascita ,studente.luogoNascita, studente.uniqueID "
          + "FROM (((studente INNER JOIN richiestatirocinio on"
          + " studente.matricola=richiestatirocinio.matricola)"
          + "INNER JOIN tirocinio on richiestatirocinio.idTirocinio=tirocinio.idTirocinio) "
          + "INNER JOIN azienda on tirocinio.p_iva=azienda.p_iva) WHERE azienda.p_iva= ?"
          + " AND richiestatirocinio.status = 'approvata'";
      preparedStatement = con.prepareStatement(query); 
