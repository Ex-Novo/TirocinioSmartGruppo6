...
public class ConvenzioneDaoImpl implements ConvenzioneDaoInterface {
  ...
  /**
   * Il metodo prende come parametro la partita iva
   *  dell'azienda di cui si vuole accettare la convenzione.
  ...
  /**
   * Il metodo prende come parametro la partita iva 
   * dell'azienda di cui si vuole rifutare la convenzione.
   * @return ritorna true se la query è andata a buon fine
   * 
  ...
  /**
   * Il metodo effettua una query per torvare tutte le aziende convenzionate con il diparimento.
  ...
  public ArrayList<Convenzione> getRichiesteConvenzione() {
    ...
    try {
      ...
      con = DBConnection.createConnection();
      String query = "SELECT dataConvenzione, stato, dettaglioConvenzione, tutorAziendale, numPosti,p_iva,nomeFile"
          + " FROM convenzione WHERE stato='in attesa'";
      preparedStatement = con.prepareStatement(query); 
  ...
  /**
   * Il metodo prende come parametro la convenzione da voler inoltrare
   *  e altri 2 parametri rappresentanti le chiavi esterne.
  ...
  public boolean invioRichiestaConvenzione(Convenzione convenzione, String email ,String piva) {
    ...
    try
    {
      ...
      String query = "INSERT INTO " + "convenzione "
          + " (dataConvenzione, stato, dettaglioConvenzione, p_iva, email, tutorAziendale, numPosti,nomeFile) "
          + "VALUES (?, ?, ?, ?, ?, ?, ?,?)";
      preparedStatement = con.prepareStatement(query);
  ...
  /**
   * Questo metodo effettua una query nel database per recuperare la richiesta di convenzione
   *  dell'azienda tramite il parametro passato della partita iva.
