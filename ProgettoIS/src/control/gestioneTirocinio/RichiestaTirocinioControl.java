...
public class RichiestaTirocinioControl extends HttpServlet {
  ...
  /**
   *  /**
   * Questo metodo salva i valori della form nel Bean richiesta tirocinio
   * e nel caso l'utente confermi la richiesta
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    if (tipo.equals("confermaForm")) {
      ...
      rTirocinio.setNomeFile("richiesta" + "_" + studente.getNome()
          + studente.getCognome() + ".pdf");
        
      session.setAttribute("rTirocinio", rTirocinio);
      session.setAttribute("studente", studente);
        
      getServletConfig().getServletContext()
        .getRequestDispatcher("/FirmaTirocinio.jsp").forward(request, response);
        
    }
      
    /* Conferma della richiesta di tirocinio
      dopo aver caricato il documento firmato salvando i dati nel database */
      
    if (tipo.equals("confermaRichiesta")
          && (boolean)session.getAttribute("fileUploaded")) {
      ...
      out.close();
    } else if (tipo.equals("confermaRichiesta")
          &&  !(boolean)session.getAttribute("fileUploaded")) {
      out.println("<script>");
